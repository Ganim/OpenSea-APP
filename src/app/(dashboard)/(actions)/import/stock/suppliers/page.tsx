...
export default function ImportSuppliersPage() {
  ...
  const entityDef = ENTITY_DEFINITIONS.suppliers;

  // Build fields - stable reference
  const enabledFields = useMemo(() => {
    const baseFields = getEntityFields('suppliers');
        ...
        <CardContent className="flex items-center gap-4 py-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-muted-foreground">
              Preencha a planilha com os dados dos fornecedores ou cole dados de
              outra planilha. Voc\u00ea tamb\u00e9m pode baixar o template Excel
              para preenchimento.
            </p>
          </div>
        </CardContent>
        ...
        <CardContent className="flex items-center justify-between py-3">
          <div className="flex items-center gap-4">
            <Badge variant="outline" className="gap-1">
              ...
              {spreadsheet.filledRowCount === 1 ? 'fornecedor' : 'fornecedores'}{' '}
              para importar
            </Badge>
            {validationResult &&
