...
import {
  ...
  getTypeBadgeVariant,
  getTypeLabel,
} from '../src/utils/permission-groups.utils';
...
export default function PermissionGroupDetailPage() {
  ...
  // Busca dados do grupo (sem popular users para evitar rbac.associations.read)
  const {
    data: groupData,
    ...
    queryFn: async () => {
      // Usar getPermissionGroupById que não tenta popular users (evita rbac.associations.read)
      const group = await rbacService.getPermissionGroupById(groupId);

      // Buscar permissões separadamente (usa rbac.groups.read)
      ...
      // Para exibição, usamos usersCount do grupo
      return {
        group,
        users: [],
        permissions,
  ...
  // Busca todas as permissões disponíveis agrupadas por módulo/recurso
  ...
  // Busca usuários do grupo separadamente (pode falhar se não tiver permissão rbac.associations.read)
  const {
    data: groupUsersData = [],
  ...
  // Extrair dados do grupo com useMemo para evitar recriação
  ...
  // Filtrar usuários do grupo pela busca
  ...
  // Filtrar usuários disponíveis (que ainda não estão no grupo)
  ...
  const addUserToGroupMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      // Forçar refetch manual para garantir atualização
  ...
  // Estrutura de permissões agrupadas por módulo/recurso com status de ativação
    ...
    <div className="min-h-screen from-purple-50 via-gray-50 to-pink-50 dark:from-gray-900 dark:via-slate-900 dark:to-slate-800 px-6 py-6">
      <div className="max-w-8xl mx-auto space-y-6">
        ...
        <div className="flex items-center gap-4">
          <div
            className="flex h-14 w-14 items-center justify-center rounded-lg bg-linear-to-br from-purple-500 to-pink-600"
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className="grid w-full grid-cols-2">
            <TabsTrigger value="users" className="flex items-center gap-2">
              ...
              Usuários ({groupData?.group?.usersCount || groupUsers.length})
            </TabsTrigger>
            <TabsTrigger
              value="permissions"
              ...
              {groupLoading ? (
                ...
                <div className="space-y-4">
                  {permissionsStructure.map(moduleGroup => (
                    <div key={moduleGroup.module} className="border rounded-lg">
                      <button
        ...
        <Card className="p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
            ...
            <div>
              ...
              <p className="font-medium">{group.priority}</p>
            </div>
          </div>
        </Card>
      </div>
