...
import type { StorageSettings } from '@/types/rbac';
...
export default function PermissionGroupEditPage() {
  ...
  const [storageAllowedTypes, setStorageAllowedTypes] = useState<string[]>([]);
  const [storageMaxFileSizeMb, setStorageMaxFileSizeMb] = useState<string>('');
  const [storageMaxStorageMb, setStorageMaxStorageMb] = useState<string>('');
  ...
  useEffect(() => {
    if (group) {
      ...
      // Storage settings
      const ss = group.storageSettings as StorageSettings | null;
      setStorageAllowedTypes(ss?.allowedFileTypes ?? []);
  ...
  const updateMutation = useMutation({
    mutationFn: async () => {
      // Build storage settings (null if everything empty)
      const hasStorageSettings =
        storageAllowedTypes.length > 0 ||
        storageMaxFileSizeMb !== '' ||
        storageMaxStorageMb !== '';
            ...
            <Card className="bg-white/5 p-6 w-full space-y-6">
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground mb-2">
                  Selecione as categorias de arquivo que os usuários deste grupo
                  podem enviar.
                </p>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                  {[
                    ...
                    >
                      <Checkbox
                        checked={storageAllowedTypes.includes(opt.value)}
                        onCheckedChange={checked => {
                          setStorageAllowedTypes(prev =>
                            checked
                              ? [...prev, opt.value]
                ...
                {storageAllowedTypes.length === 0 && (
                  <p className="text-xs text-amber-600 dark:text-amber-400">
                    Nenhum tipo selecionado — todos os tipos serão permitidos.
                  </p>
                )}
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    min="1"
                    max="500"
                    value={storageMaxFileSizeMb}
                    onChange={e => setStorageMaxFileSizeMb(e.target.value)}
                    placeholder="Ex: 50"
                  />
                  <p className="text-xs text-muted-foreground">
                    Deixe vazio para usar o limite padrão (500 MB).
                  </p>
                </div>

                <div className="space-y-2">
                  <Label htmlFor="maxStorageMb">
                    Quota total de armazenamento (MB)
                  </Label>
                  <Input
                    id="maxStorageMb"
                    type="number"
                    min="1"
                    value={storageMaxStorageMb}
                    onChange={e => setStorageMaxStorageMb(e.target.value)}
                    placeholder="Ex: 1024"
                  />
                  <p className="text-xs text-muted-foreground">
                    Limite total de armazenamento para usuários do grupo. Deixe
