 ...
 * Página de listagem de categorias de produtos
 */
...
function ProductCategoriesContent() {
  ...
  // Filtrar apenas categorias de produto
  const productCategories = useMemo(
    () => allCategories.filter(cat => !cat.slug?.includes('template')),
    [allCategories]
  );
  ...
  const handleCategoryDoubleClick = (id: string) => {
    router.push(`/stock/assets/product-categories/${id}`);
  };

  const handleCategoriesView = (ids: string[]) => {
    if (ids.length === 1) {
      router.push(`/stock/assets/product-categories/${ids[0]}`);
    } else {
  ...
  const handleCategoriesEdit = (ids: string[]) => {
    if (ids.length === 1) {
      router.push(`/stock/assets/product-categories/${ids[0]}/edit`);
    } else {
  ...
  const stats = [
    ...
    {
      label: 'Ativas',
      value: productCategories.filter(c => c.isActive).length,
    ...
    },
    {
      ...
      value: productCategories.filter(c =>
  ...
  const pageHeaderConfig: PageHeaderConfig = {
    ...
    onAdd: () => router.push('/stock/assets/product-categories/new'),
  ...
  return (
    <div className="flex-col flex gap-4">
      ...
      <EntityGrid
        ...
        emptyMessage="Nenhuma categoria de produto cadastrada"
