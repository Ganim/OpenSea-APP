...
export const COMMENT_QUERY_KEYS = {
  COMMENTS: (boardId: string, cardId: string) => ['task-comments', boardId, cardId],
} as const;

export function useComments(boardId: string, cardId: string, params?: CommentsQuery) {
  return useQuery({
...
export function useCreateComment(boardId: string, cardId: string) {
  ...
  return useMutation({
    mutationFn: (data: CreateCommentRequest) => commentsService.create(boardId, cardId, data),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: COMMENT_QUERY_KEYS.COMMENTS(boardId, cardId) });
...
export function useDeleteComment(boardId: string, cardId: string) {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (commentId: string) => commentsService.delete(boardId, cardId, commentId),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: COMMENT_QUERY_KEYS.COMMENTS(boardId, cardId) });
      qc.invalidateQueries({ queryKey: CARD_QUERY_KEYS.CARDS(boardId) });
      qc.invalidateQueries({ queryKey: CARD_QUERY_KEYS.CARD(boardId, cardId) });
